                      ...
                      if (!isset($opt['file']))
                        {
                          ...
                          if ($this->play)
                            ...
                            {
                              if ($opt['filesize'])
                                  $outFile = JoinUrl($this->outDir, $this->outFile . '-' . $this->fileCount++);
                              else
                                  $outFile = JoinUrl($this->outDir, $this->outFile);
                            }
                          else
                            {
                              if ($opt['filesize'])
                                  $outFile = JoinUrl($this->outDir, $this->baseFilename . '-' . $this->fileCount++);
                              else
                                  $outFile = JoinUrl($this->outDir, $this->baseFilename);
                            }
                          $this->InitDecoder();
          ...
          if (!isset($opt['flv']))
            {
              ...
              if ($filesize)
                  $opt['flv'] = WriteFlvFile(JoinUrl($outDir, $outFile . '-' . $fileCount++), $f4f->audio, $f4f->video);
              else
                  $opt['flv'] = WriteFlvFile(JoinUrl($outDir, $outFile), $f4f->audio, $f4f->video);
              if ($metadata)
