    ...
    for (var timestamp in messages) {
      if (timestamp == 0) {
        ...
        if (_chatblock_response_timestamps.indexOf(timestamp) > -1 ) {
          ...
          $("#chatblock_block").children("br").before('<div style="width: 95%; display: block;"><span style="color: red">' + messages[timestamp]['username'] + ': </span> ' + messages[timestamp]['message'] + '</div>');
