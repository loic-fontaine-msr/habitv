...
public class LEquipeRetreiver {
	...
	public static Set<EpisodeDTO> findEpisodeByCategory(
		...
		String baseUrl = LEquipeConf.VIDEO_HOME_URL + category.getId();
	...
	private static void findEpisodeByUrl(final CategoryDTO category,
			final Set<EpisodeDTO> episodeList, String pageUrl) {
		try {
			final Connection con = Jsoup.connect(pageUrl);
			...
			if (elementsByClass.isEmpty()) {
				...
				for (final Element aResult : elementsByClass.get(0).children()) {
					...
					final String name = aResult.getElementsByClass("title")
							.text();
	...
	public static String findDownloadlink(final String url) {
		...
		Pattern pattern = Pattern
				.compile("<param name=\"flashVars\" value=\"language_code=fr&amp;playerKey=(.*)&amp[&amp]*;suffix=&amp;sig=(.*)\">");
	...
	private static String findDownloadlinkBySigAndPlayerKey(final String sig,
		...
		return "\"http://api.kewego.com/video/getStream/?appToken=" + token
				+ "&sig=" + sig + "&format=w640&v=2749\"";
	}

	public static Set<CategoryDTO> findCategory() {
		try {
			final Set<CategoryDTO> categoryDTOs = new HashSet<>();
			final Connection con = Jsoup.connect(LEquipeConf.VIDEO_HOME_URL);

			final Elements aOngletsSport = con.get().select(".onglets_sports")
					.get(0).children();
			for (final Element aHref : aOngletsSport) {
				...
				if (href.length() > 1) {
					...
					final CategoryDTO categoryDTO = new CategoryDTO(
							LEquipeConf.NAME, content, href,
							LEquipeConf.EXTENSION);
	...
	private static Collection<CategoryDTO> findSubCategories(
		...
		try {
			...
			final Elements aOngletsSport = con.get().select("#k_sous").get(0)
					.children();
			for (final Element aHref : aOngletsSport) {
				final String href = aHref.attr("href");
				if (href.length() > 1) {
